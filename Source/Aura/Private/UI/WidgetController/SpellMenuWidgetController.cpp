...
void USpellMenuWidgetController::BindCallbacksToDependencies()
{
	GetAuraASC()->AbilityStatusChangedDelegate.AddLambda([this](const FGameplayTag& AbilityTag, const FGameplayTag& StatusTag, int32 NewLevel)
	{
		if (SelectedAbility.Ability.MatchesTagExact(AbilityTag))
		{
			...
			SpellGlobeSelectedDelegate.Broadcast(bEnableSpendPoints, bEnableEquipButton, Description, NextLevelDescription);
		}
		if (AbilityInfo)
		{
...
void USpellMenuWidgetController::SpellGlobeSelected(const FGameplayTag& AbilityTag)
{
	if (bWaitingForEquipSelection)
	{
		...
		StopWaitingForEquipSelectedDelegate.Broadcast(SelectedAbilityType);
	...
	bool bEnableSpendPoints = false;
	bool bEnableEquipButton = false;
	ShouldEnableButtons(AbilityStatus, SpellPoints, bEnableSpendPoints, bEnableEquipButton);
	FString Description;
	FString NextLevelDescription;
	GetAuraASC()->GetDescriptionsByAbilityTag(AbilityTag, Description, NextLevelDescription);
	SpellGlobeSelectedDelegate.Broadcast(bEnableSpendPoints, bEnableEquipButton, Description, NextLevelDescription);
...
void USpellMenuWidgetController::EquipButtonPressed()
{
	...
	WaitForEquipSelectedDelegate.Broadcast(AbilityType);
...
void USpellMenuWidgetController::SpellRowGlobePressed(const FGameplayTag& SlotTag, const FGameplayTag& AbilityType)
{
	...
	// Check selected ability against the slot's ability type (don't equip offensive spell in passive slot, vice versa)
	const FGameplayTag& SelectedAbilityType = AbilityInfo->FindAbilityInfoForTag(SelectedAbility.Ability).AbilityType;
	...
	const FGameplayTag& Slot, const FGameplayTag& PreviousSlot)
{
	...
	StopWaitingForEquipSelectedDelegate.Broadcast(AbilityInfo->FindAbilityInfoForTag(AbilityTag).AbilityType);
